...
const BlogRollProjectsTelecom = ( {data} ) => {
...
BlogRollProjectsTelecom.propTypes = {
...
export default () => (
  <StaticQuery
    query={graphql`
      query BlogRollProjectsTelecomQuery {
        allMarkdownRemark(
          sort: { order: DESC, fields: [frontmatter___date] }
          filter: { frontmatter: {featuredprojects: {eq: true}, category: {eq: "telecom"} } }
        ) {
          edges {
            node {
    ...
    `}
    render={(data, count) => <BlogRollProjectsTelecom data={data} count={count} />}
  />
)
