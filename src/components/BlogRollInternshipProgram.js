...
const BlogRollInternshipProgram = ( {data} ) => {
...
BlogRollInternshipProgram.propTypes = {
...
export default () => (
  <StaticQuery
    query={graphql`
      query BlogRollInternshipProgramQuery {
        allMarkdownRemark(
          sort: { order: DESC, fields: [frontmatter___date] }
          filter: { frontmatter: {featuredblog: {eq: true}, category: {eq: "internship-program"} } }
        ) {
          edges {
            node {
    ...
    `}
    render={(data, count) => <BlogRollInternshipProgram data={data} count={count} />}
  />
)
