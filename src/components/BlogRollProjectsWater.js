...
const BlogRollProjectsWater = ( {data} ) => {
...
BlogRollProjectsWater.propTypes = {
...
export default () => (
  <StaticQuery
    query={graphql`
      query BlogRollProjectsWaterQuery {
        allMarkdownRemark(
          sort: { order: DESC, fields: [frontmatter___date] }
          filter: { frontmatter: {featuredprojects: {eq: true}, category: {eq: "water"} } }
        ) {
          edges {
            node {
    ...
    `}
    render={(data, count) => <BlogRollProjectsWater data={data} count={count} />}
  />
)
