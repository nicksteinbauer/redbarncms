...
export default () => (
  <StaticQuery
    query={graphql`
      query BlogRollAwardsQuery {
        allMarkdownRemark(
          sort: { order: DESC, fields: [frontmatter___date] }
          filter: { frontmatter: {featuredblog: {eq: true}, category: {eq: "awards"} } }
        ) {
          edges {
            node {
    ...
    `}
    render={(data, count) => <BlogRollAwards data={data} count={count} />}
  />
)
