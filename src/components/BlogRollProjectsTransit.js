...
const BlogRollProjectsTransit = ( {data} ) => {
...
BlogRollProjectsTransit.propTypes = {
...
export default () => (
  <StaticQuery
    query={graphql`
      query BlogRollProjectsTransitQuery {
        allMarkdownRemark(
          sort: { order: DESC, fields: [frontmatter___date] }
          filter: { frontmatter: {featuredprojects: {eq: true}, category: {eq: "transit"} } }
        ) {
          edges {
            node {
    ...
    `}
    render={(data, count) => <BlogRollProjectsTransit data={data} count={count} />}
  />
)
