...
const BlogRollIntegratedCulture = ( {data} ) => {
...
BlogRollIntegratedCulture.propTypes = {
...
export default () => (
  <StaticQuery
    query={graphql`
      query BlogRollIntegratedCultureQuery {
        allMarkdownRemark(
          sort: { order: DESC, fields: [frontmatter___date] }
          filter: { frontmatter: {featuredblog: {eq: true}, category: {eq: "integrated-culture"} } }
        ) {
          edges {
            node {
    ...
    `}
    render={(data, count) => <BlogRollIntegratedCulture data={data} count={count} />}
  />
)
