...
const BlogRollSustainability = ( {data} ) => {
...
BlogRollSustainability.propTypes = {
...
export default () => (
  <StaticQuery
    query={graphql`
      query BlogRollSustainabilityQuery {
        allMarkdownRemark(
          sort: { order: DESC, fields: [frontmatter___date] }
          filter: { frontmatter: {featuredblog: {eq: true}, category: {eq: "sustainability"} } }
        ) {
          edges {
            node {
    ...
    `}
    render={(data, count) => <BlogRollSustainability data={data} count={count} />}
  />
)
