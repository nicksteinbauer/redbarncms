...
const BlogRollTechnicalExcellence = ( {data} ) => {
...
BlogRollTechnicalExcellence.propTypes = {
...
export default () => (
  <StaticQuery
    query={graphql`
      query BlogRollTechnicalExcellenceQuery {
        allMarkdownRemark(
          sort: { order: DESC, fields: [frontmatter___date] }
          filter: { frontmatter: {featuredblog: {eq: true}, category: {eq: "technical-excellence"} } }
        ) {
          edges {
            node {
    ...
    `}
    render={(data, count) => <BlogRollTechnicalExcellence data={data} count={count} />}
  />
)
